...
class MosaicV1():
    ...
    def mosaic(self, results):
        ...
        # get min shape
        min_h = min(a['img'].shape[0], b['img'].shape[0],
                    c['img'].shape[0], d['img'].shape[0])
        min_w = min(a['img'].shape[1], b['img'].shape[1],
                    c['img'].shape[1], d['img'].shape[1])
        ...
        for key in ['gt_labels', 'gt_bboxes', 'gt_labels_ignore', 'gt_bboxes_ignore']:
            if key in results:
                results[key] = np.concatenate(
                    [a[key], b[key], c[key], d[key]], axis=0)
        ...
        if random.random() < self.crop_probability:
            ...
            resize = MMDCompose(
                [dict(type='Resize', img_scale=(min_h, min_w))])
...
class MosaicV2():
    ...
    def mosaic(self, results):
        ...
        # cropping pipe
        random_crop = MMDCompose(
            [dict(type='RandomCrop', crop_size=(min_h, min_w))])

        # crop
        a, b, c, d = random_crop(a), random_crop(
            b), random_crop(c), random_crop(d)
        ...
        cut_x = random.randint(int(min_h * min_offset),
                               int(min_h * (1 - min_offset)))
        cut_y = random.randint(int(min_w * min_offset),
                               int(min_w * (1 - min_offset)))

        # crop again to mosaic size - now it's not a random crop tho
        # cropping pipe
        crop = MMDCompose(
            [dict(type='Crop', crop_size=(cut_x, cut_y), crop_offset=(0, 0))])
        a = crop(a)
        crop = MMDCompose(
            [dict(type='Crop', crop_size=(cut_x, min_w - cut_y), crop_offset=(0, cut_y))])
        b = crop(b)
        crop = MMDCompose(
            [dict(type='Crop', crop_size=(min_h - cut_x, cut_y), crop_offset=(cut_x, 0))])
        c = crop(c)
        crop = MMDCompose([dict(type='Crop', crop_size=(
            min_h - cut_x, min_w - cut_y), crop_offset=(cut_x, cut_y))])
        d = crop(d)
        ...
        # added crop & concatenating masks - hjs
        # top_masks = np.concatenate([a['masks'], b['masks']], axis=1)
        # bottom_masks = np.concatenate(c['masks'], d['masks'], axis=1)
        # results['masks'] = np.concatenate([top_masks, bottom_masks], axis=0)
        # results['masks_shape'] = (min_h * 2, min_w * 2)
        ########################################
        for key in ['gt_labels', 'gt_bboxes',  'gt_labels_ignore', 'gt_bboxes_ignore']:
            if key in results:
                results[key] = np.concatenate(
