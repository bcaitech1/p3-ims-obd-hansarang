...
def train(data_dir, model_dir, args):
    ...
    # -- dataset
    dataset_module1 = getattr(import_module("dataset"), args.dataset)
    dataset_module2 = getattr(import_module("dataset"), 'PseudoTrainset')# default: BaseAugmentation
    train_dataset = dataset_module2(
        data_dir=train_path,
        mode='train'
    )
    val_dataset = dataset_module1(
        data_dir=val_path,
        mode='val'
    )
    ...
    # model_path = './model/final_with_multiloss/best.pth'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='final_with_multiloss', help='model save at {SM_MODEL_DIR}/{name}')

    # Container environment
    args = parser.parse_args()

    wandb.run.name = 'final_multiloss'
